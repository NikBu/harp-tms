        ...
        <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground">
                <tr className="border-b border-border">
                    <th className="px-3 py-2 text-left">{t('navigation.projects')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.activity.new_cases')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.activity.updated_cases')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.activity.new_results')}</th>
                </tr>
            </thead>
            <tbody>
                {rows.map(([name, data]) => (
        ...
        <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground">
                <tr className="border-b border-border">
                    ...
                    <th className="px-3 py-2 text-right">{t('reports.dashboard.coverage_pct')}</th>
                </tr>
            </thead>
            <tbody>
                {rows.map(([name, data]) => (
                ...
                <tr className="border-b border-border">
                    ...
                    {PRIORITY_KEYS.map((k) => (
                        <th key={k} className={`px-3 py-2 text-right ${PRIORITY_COLORS[k]}`}>
                            {t(`test_cases.priorities.${k}`)}
                        </th>
                    ))}
                    <th className="px-3 py-2 text-right">{t('reports.distribution.total')}</th>
                </tr>
            </thead>
            <tbody>
                {rows.map(([name, data]) => (
    ...
    return (
        <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground">
                <tr className="border-b border-border">
                    <th className="px-3 py-2 text-left">{t('navigation.projects')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.milestone.total')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.milestone.completed')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.milestone.active')}</th>
    ...
    return (
        <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground">
                <tr className="border-b border-border">
                    <th className="px-3 py-2 text-left">{t('navigation.projects')}</th>
                    <th className="px-3 py-2 text-right">{t('reports.workload.members')}</th>
                    ...
                    <th className="px-3 py-2 text-right">{t('reports.workload.avg_pass_rate')}</th>
                </tr>
            </thead>
            <tbody>
                {rows.map(([name, data]) => {
...
export default function CrossProjectReport({
    ...
    function renderTable() {
        switch (type) {
            ...
            default: return <p className="text-sm text-muted-foreground">{t('reports.coming_soon')}</p>;
        }
    }
    ...
    return (
        <>
            <Head title={t('reports.cross.title')} />
            <div className="flex h-full flex-1 flex-col gap-6 p-4">
                ...
                <Card>
                    ...
                    <CardContent className="overflow-x-auto">
                        {rows.length === 0
                            ? <p className="py-6 text-center text-sm text-muted-foreground">{t('projects.empty_title')}</p>
                            : renderTable()
                        }
                    </CardContent>
                </Card>
