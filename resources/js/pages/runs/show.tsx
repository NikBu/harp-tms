            ...
            <DialogContent>
                <form onSubmit={submit} className="grid gap-4">
                    ...
                    <div className="grid gap-2">
                        <Label>{t('runs.result.status')}</Label>
                    ...
                    {/* Comment */}
                    <div className="grid gap-2">
                        <Label htmlFor="comment">{t('runs.result.comment')}</Label>
                        <RichTextEditor
                            value={data.comment}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-2">
                            <Label htmlFor="elapsed">{t('runs.result.elapsed')}</Label>
                            <input
                                id="elapsed"
                                value={data.elapsed}
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="version">{t('runs.result.version')}</Label>
                            <input
                                id="version"
                                value={data.version}
    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="max-w-lg">
                <form onSubmit={submit} className="grid gap-4">
                    <DialogHeader>
                        <DialogTitle>{t('runs.bulk.title')}</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="grid gap-2">
                            <Label htmlFor="bulk-version">{t('runs.result.version')}</Label>
                            <input
                                id="bulk-version"
                                value={version}
                    ...
                    {/* Comment */}
                    <div className="grid gap-2">
                        <Label htmlFor="bulk-comment">{t('runs.result.comment')}</Label>
                        <RichTextEditor
                            value={comment}
                            onChange={setComment}
                        />
                    ...
                    <div className="grid gap-1.5">
                        <div className="flex items-center justify-between">
                            <Label>{t('runs.bulk.select_tests')}</Label>
                            <button
                                type="button"
                                onClick={toggleAll}
                        ...
                        <p className="text-xs text-muted-foreground">
                            {selectedIds.size} {t('runs.bulk.selected')}
                            {untested.length > 0 && (
                                <button
                                    type="button"
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={processing || selectedIds.size === 0}>
                            {t('runs.bulk.submit', { count: String(selectedIds.size) })}
                        </Button>
                    </DialogFooter>
                </form>
            </DialogContent>
...
export default function RunsShow({
    ...
    const grouped = (run.tests ?? []).reduce<Record<string, TestInstance[]>>((acc, test) => {
        const key = test.case?.section?.name ?? t('test_cases.no_section');
        acc[key] = acc[key] ?? [];
        acc[key].push(test);
        return acc;
    }, {});
                ...
                <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                        ...
                        <Button variant="outline" onClick={toggleClose}>
                            {run.is_completed
                                ? <><XCircle className="size-4" /> {t('runs.reopen')}</>
                                : <><CheckCircle2 className="size-4" /> {t('runs.close')}</>
                            }
                        </Button>
